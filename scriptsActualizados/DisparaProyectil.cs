...
public class DisparaProyectil : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision){ // y esto es para ver las colisiones de la bala 
        ...
        if(collision.gameObject.tag=="portalRegresoBosqueCueva" ||
            collision.gameObject.tag == "portalBosqueAbajo" ||
            collision.gameObject.tag == "portalRegresoBosqueMansion" ||
            collision.gameObject.tag == "portalEntradaMansion" ||
            collision.gameObject.tag == "portalBosqueDer" ||
            collision.gameObject.tag == "portalBosqueCueva" ||
            collision.gameObject.tag == "portalCiudadDer" ||
            collision.gameObject.tag == "portalCiudadAbajo" ||
            collision.gameObject.tag == "portalParqueIzq" ||
            collision.gameObject.tag == "portalParqueArriba" ||
            collision.gameObject.tag == "portalCasaUnicaArriba" ||
            collision.gameObject.tag == "portalSalidaMansion"){
            Destroy(this.gameObject);
        }
    }
}
